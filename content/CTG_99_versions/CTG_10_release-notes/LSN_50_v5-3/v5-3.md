...
- Backported fix on UI `val` JS function for single or multiple emum fields for select2-based renderings
...
- Backported fix to prevent implicit fallback to unauthenticated user when calling the API endpoint
  with a malformed authorization header (e.g. ommiting the `Bearer` auth type prefix in the `Authorization/X-Simplicite-Authorization` header value)
...
### 5.3.29 (UNRELEASED) - maintenance revision <span id="version-5.3.29"></span>

- Force UI to reload metadata on CONTEXT_REFSELECT
