...
### 5.3.59 (UNRELEASED) - maintenance revision <span id="version-5.3.59"></span>

- Backported improvements on the developments artifacts of the exported/committed module (StyleLint for CSS/LESS code)
- Fix search access when md.search is "none"
- Fix missing ENUM access on menu when the field is read-only
