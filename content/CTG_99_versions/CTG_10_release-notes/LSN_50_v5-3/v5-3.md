...
### 5.3.39 (UNRELEASED) - maintenance revision <span id="version-5.3.39"></span>

- Backported fix on color picker of theme editor
- Backported loading of SAML attributes intto the session info
- Backported SAML config flexibility: allowed base64-encoded SAML certificate/keys in AUTH_PROVIDERS
- Fixed `initAction` call for state transition with confirmed action containing fields 
