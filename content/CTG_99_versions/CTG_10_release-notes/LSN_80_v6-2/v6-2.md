...
> **Warning**: This release note corresponds to the **next** minor version which is still in **BETA** stage and thus will be regularly amendend.
> <br/>**BETA** stage means it is **not yet suitable** for production
...
### 6.2.0 release candidate 2 (2025-01-31) <span id="version-6.2.0-beta-RC2"></span>
...
### 6.2.0 release candidate 3 (UNRELEASED) <span id="version-6.2.0-beta-RC3"></span>

- Fixed regression on DB access
- Used secure random instead of plain random generator
- Security fix in `JSONTool.jsonUser`
- Various minor fixes
- Fix UI issue with inlined 0,1 link: its child lists will be hidden to preserve only one parent object in form
