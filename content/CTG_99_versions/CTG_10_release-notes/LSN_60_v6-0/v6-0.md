...
### 6.0.21 (UNRELEASED) - maintenance revision <span id="version-6.0.21"></span>

- Backported fix: NPE when comparing module with a remote JSON format
- Backported fix: remove button of user-filter badge in case of a foreign-key
- Backported fix: predef-search update in session cache
- Backported fix: save undo/redo updates in RedoLog
- Backported fix: missing `ActionGroup` and `TranslateActionGroup` in module export JSON
