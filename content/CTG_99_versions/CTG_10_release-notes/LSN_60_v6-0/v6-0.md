...
- Backported improvements and fixes on class field checks for disposition, adpater, internal and external objects and workflows
...
- Backporte fix on regression for transition buttons styles
- Backported add of type in action metadata jSON (used by template editor)
- Backported fixes on the themplate editor vs embedded transition action buttons
...
- Backported new widgets: `ChartjsExternalObject` (for arbitrary Chart.js charting)
  and `MustacheExternalObject` (for arbirary Mustache template redendring)
...
- `CHARTJS_VERSION` set by default to `3` (should have been done since first release of version 6).
  If you experiment issues with your custom pages using Chart.js v2 you can force the overriden value of this system parameter to `2`
...
- Backported fix on HTML editor in a dialog box

### 6.0.12 (UNRELEASED) <span id="version-6.0.12"></span>
