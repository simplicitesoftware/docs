...
- Experimental JSON schema validation support for some settings (e.g. on REST mappend external object services)
...
### 6.0.3 (UNRELEASED) <span id="version-6.0.3"></span>
...
- New template editor option for date to set the property `data-autoopen` to auto-open the date picker when focusing the input
