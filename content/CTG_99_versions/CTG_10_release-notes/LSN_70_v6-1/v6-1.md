...
### 6.1.11 (UNRELEASED) <span id="version-6.1.11"></span>

- WCAG accessibility improvements (table, datetime picker...)
- Supports paginated search with previous V5 syntax (without pagenum in lambda function)
- Force the search position to popup when the list has a row template (no search by column)
