...
- Allows update on simple referenced fields (not user key) thru the parent list or form
...
- Edit row template: inhibited minified/grouped list to edit rows only
...
- Backported change on ping and health check to return a `503` instead of a `500` in case of database unavailability
