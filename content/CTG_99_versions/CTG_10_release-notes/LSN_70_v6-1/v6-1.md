...
### 6.1.22 (UNRELEASED) - maintenance revision <span id="version-6.1.22"></span>

- Backported security fix in `JSONTool.jsonUser`
- Backported fix contextual values on panel before calling `initRefSelect` hook
- Backported fix `AppLog` in DB with related asynchronous task
