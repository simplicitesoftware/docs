...
- foreign-keys per link:
	...
	- non unique index on meta-object fields (logical link to `<object>:<id>`): syntax `<table>_<coluln>_mo`
...
The engine generate the `ALTER` of tables and columns with the name/type/length is updated during module import.
It replace also the indexes and foreign-keys.
