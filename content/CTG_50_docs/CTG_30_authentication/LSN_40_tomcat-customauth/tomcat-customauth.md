<<<<<<< HEAD:content/CTG_50_docs/CTG_30_authentication/LSN_40_tomcat-customauth/tomcat-customauth.md
...
When using an Apache or NGINX webserver as reverse proxy you can configure it to enforce client certificate authentication (see [this document](/resource/docs/misc/webserver-ssl) for details).
...
Among the provided OAuth2 authentication providers, the `simplicite` provider acts like the traditional database realm authentication mechanism.
You can thus have a custom authentication co-existing with the usual password-based authentication. 
>>>>>>> rec:content/CTG_50_docs/CTG_80_authentication/LSN_40_tomcat-customauth/tomcat-customauth.md
